...
describe('client bundle verification', () => {
  it('excludes NestJS and the real controller implementation from the production bundle', async () => {
    ...
    // "responseType: blob" request property is a single-use constant here,
    // so the minifier is free to inline it away; `.blob()` is a real method
    // name and survives minification regardless.
    expect(bundleContents).toContain('/files/');
    expect(bundleContents).toContain('encodeURIComponent');
    expect(bundleContents).toContain('.blob()');
  });
});
